...
class Display(): #Representa el juego por pantalla
    ...
    def analyze_events(self, color, direction):
        events = []
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN: 
                if event.key == pygame.K_ESCAPE: #Esc keyboard
                    events.append("quit")
                elif event.key == pygame.K_UP and direction != "down": #si me estoy moviendo hacia abajo y quiero ir hacia arriba no puedo
                    events.append("up")
                elif event.key == pygame.K_DOWN and direction != "up": #si me estoy moviendo hacia arriba y quiero ir hacia abajo no puedo
                    events.append("down")
                elif event.key == pygame.K_RIGHT and direction != "left": #si me estoy moviendo hacia la derecha y quiero ir hacia la izquierda no puedo
                    events.append("right")
                elif event.key == pygame.K_LEFT and direction != "right": #si me estoy moviendo hacia derecha y quiero ir hacia la izquierda no puedo
                    events.append("left")
    ...
    def finDelJuego(self, i , pantalla): #Para mostrar la pantalla cuando hay game_over
       ...
       # Obtengo las dimensiones de la pantalla
        ...
        if i == 1: #Gana BLACK snake
            ...
            winner_rect = winner_surface.get_rect()
            winner_rect.midtop = (pantalla[X]//2, pantalla[Y]//2)
            pantalla.blit(winner_surface, winner_rect) 
    ...
    def refresh(self): #Para actualizar la pantalla del juego
    ...
    try:
        with Client((ip_address, 6000), authkey=b'secret password') as conn:
            ...
            while game.is_running(): 
                ...
                pantalla.fill(BLACK)
                events = display.analyze_events(color,direction)
                for ev in events:
                    conn.send(ev)
                    if ev == "quit":
                        game.stop()
